...
when is_main_module:
    import json, skbintext

    const
        blake2_cases = slurp"../../t/blake2-kat.json"

    echo("TAP version 13")
    echo("# Blake2s hashes")

    var buffer: seq[uint8]
    new_seq(buffer, uint64.sizeof * 4)

    var js = parseJson(blake2_cases)
    var tests = 0
    for testcase in js:
        if testcase["hash"].get_str() == "blake2s":
            ...
            zeromem(addr buffer[0], uint32.sizeof * 8)
            inc tests
